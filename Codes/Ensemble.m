...
%InconelTested = regressionEnsemble.predict(InconelTest(:, 1:end - 1)); 
InconelTested = predict(regressionEnsemble.Trained{1}, InconelTest(:, 1:end - 1));
fprintf('\nEnsemble Method\n');
...
%AluminiumTested = regressionEnsemble.predict(AluminiumTest(:, 1:end - 1)); 
AluminiumTested = predict(regressionEnsemble.Trained{1}, AluminiumTest(:, 1:end - 1));
fprintf('\nEnsemble Method\n');
...
%SteelTested = regressionEnsemble.predict(SteelTest(:, 1:end - 1)); 
SteelTested = predict(regressionEnsemble.Trained{1}, SteelTest(:, 1:end - 1));
fprintf('\nEnsemble Method\n');
...
TotalTested = predict(regressionEnsemble.Trained{1}, TotalTestValue(:, 1:end - 1));
